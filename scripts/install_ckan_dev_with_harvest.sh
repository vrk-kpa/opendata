...
if $FROM_SOURCE; then
	...
	# Make sure virtualenv's copies of commands like paster is used
	deactivate
	. $VIRTUAL_ENVIRONMENT/bin/activate
...
echo "Setting up Solr, overwriting existing config file"
...
if $FROM_SOURCE; then
	...
	sudo apt-get -y install redis-server python-pip git-core
