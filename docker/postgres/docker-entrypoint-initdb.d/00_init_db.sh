...
# init prefect database
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE ROLE $DB_PREFECT_USER LOGIN PASSWORD '$DB_PREFECT_PASS';
    CREATE DATABASE $DB_PREFECT OWNER $DB_PREFECT_USER ENCODING 'utf-8';
    GRANT ALL PRIVILEGES ON DATABASE $DB_PREFECT TO $DB_PREFECT_USER;
EOSQL
